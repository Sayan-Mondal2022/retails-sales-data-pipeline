...
# MAGIC
# MAGIC I found **243 records** with duplicate **Transaction IDs**. The next step is to verify whether the remaining column values are identical for records sharing the same Transaction ID.
...
# DBTITLE 1,Plotting barplot for duplicate values
import pandas as pd
...
# MAGIC
# MAGIC There are **6 customer names** that are linked to more than one **Customer ID**.
...
# MAGIC
# MAGIC - **34 records** contain invalid quantity values.
# MAGIC - **4,209 records** contain valid quantity values.
# MAGIC
...
# DBTITLE 1,Validating the Discount Column
